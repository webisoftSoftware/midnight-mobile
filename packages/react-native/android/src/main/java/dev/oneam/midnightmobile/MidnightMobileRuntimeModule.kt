...
private fun runtimeException(error: Throwable): CodedException {
  val code = when (error) {
    ...
    is MidnightRuntimeException.InsufficientFunds -> "INSUFFICIENT_FUNDS"
    is MidnightRuntimeException.BalanceApprovalChanged -> "BALANCE_APPROVAL_CHANGED"
    is MidnightRuntimeException.UnsupportedTransaction -> "UNSUPPORTED_TRANSACTION"
